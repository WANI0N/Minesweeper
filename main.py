...
def play(self, random=0):
    ...
    while (xi < self.gridSize.w):
        ...
        while (yi < self.gridSize.h):
            if isinstance(self.gridData[xi][yi], int) and \
                    (self.gridData[xi][yi] != 0):
                # retrieve array of unknown (x) cells
                # from 8 possible cell neighbours
                unknown = win.getNeighbour(self, xi, yi, "x")
                mines = win.getNeighbour(self, xi, yi, "m")  # same for mines
                # if number of mines+unknown equals target cell number
                if (len(unknown)+len(mines) == self.gridData[xi][yi]):
                    for cell in unknown:  # mark mines
                        self.gridData[cell[0]][cell[1]] = "m"
                # win.click(self,cell[0],cell[1],1) #mark mines for debugging,
                # else disable for speed boost
                ...
                if (len(mines) == self.gridData[xi][yi]):
                    for cell in unknown:
                        ...
                        if (self.gameOver == 1):
                            return False
                        scan = 1
                # scan area (actually via getPixel if valid scan
                # (or click has been made))
                if (scan == 1):
...
r = False
while r is False:
    # innitiate winData object; retreive coordinates of target window,
    # creates map (winData.GridData), declares top left cell position
    winData = win(hwnd)
